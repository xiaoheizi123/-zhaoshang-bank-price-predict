...
path = "./"
def read_data_train():        #读Y1、Y2、Y3的训练数据，成list格式
	...
	for i in range(2530,7766):
		...
		for j in range(45):
			...
			elif(sheet_train.cell_value(0,j) in ['美国:GDP:现价:环比折年率:季调', '欧元区:GDP平减指数:季调',  \
				'工业增加值:当月同比', '工业增加值:累计同比', '固定资产投资完成额:累计同比', '房地产开发投资完成额:累计同比', '产量:发电量:当月同比', '产量:发电量:累计同比', \
				'M0:同比', 'M1:同比', 'M2:同比','CPI:当月同比', 'PPI:全部工业品:当月同比','RPI:当月同比', '上证综合指数']):
...
def read_data_test():     #读测试数据集，用于预测想要的结果
	...
	for i in range(2,826):
		...
		for j in range(45):
			...
			if(j==0):
			...
			elif(sheet_test.cell_value(0,j) in ['美国:GDP:现价:环比折年率:季调', '欧元区:GDP平减指数:季调',  \
				'工业增加值:当月同比', '工业增加值:累计同比', '固定资产投资完成额:累计同比', '房地产开发投资完成额:累计同比', '产量:发电量:当月同比', '产量:发电量:累计同比', \
				'M0:同比', 'M1:同比', 'M2:同比','CPI:当月同比', 'PPI:全部工业品:当月同比','RPI:当月同比', '上证综合指数']):
				if sheet_test.cell_value(i,j)=='':
					...
					if(M==0):
						for p in range(200):
							if sheet_test.cell_value(i-p,j) != '':
								item_content_test.append(sheet_test.cell_value(i-p,j) )
								M=1
								break
...
#http://xgboost.readthedocs.io/en/latest/python/python_api.html
    ...
    #测试Y1时用的代码，因为Y1、Y2、Y3有共同使用的变量，所以当测试一个数据时，另两个需要注释掉，以下同理
	...
	# clf1=LinearRegression(fit_intercept=True)                                              #线性回归方法
 ...
 # 	clf2=LinearRegression(fit_intercept=True)
	...
	# result3.to_csv(path+'Y11.txt',index=False)


	result3=pd.DataFrame(preds3)
	result3.to_csv(path+'sublime_Y3.txt',index=False)
	result3_lin=pd.DataFrame(pred_R3)
	result3_lin.to_csv(path+'sublime_Y3_lin.txt',index=False)
	result3_fore=pd.DataFrame(pred_L3)
	result3_fore.to_csv(path+'sublime_Y3_fore.txt',index=False)
